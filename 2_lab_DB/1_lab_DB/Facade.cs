...
namespace _1_lab_DB
{
    internal class Facade
    {
        ...
        {
            //Георгий Васильевич Кондратов,_,_,_
            Console.WriteLine("Введите список значений атрибутов для поиска модели. Примечание: разделителями между атрибутами считать запятые, если на месте пропущенных атрибутов ставить прочерк(нижнее подчеркивание)");
            string line = Console.ReadLine();
            Console.WriteLine("Введите параметры количества выдаваемых результатов(INT) и смещение(INT) через пробел");
            string s = Console.ReadLine();
            string[] dataAtribut = line.Split(',');
            string[] dataParam = s.Split(' ');
            List<Doctor> doctors = _mapper.Search(dataAtribut, Int32.Parse(dataParam[0]));
            int index = Int32.Parse(dataParam[1]);
            for(int i = 0;i < doctors.Count;i++)
            {
